...
class Memory:
    ...
    @staticmethod
    def read_last_n_lines_with_json_loads(file_path: str, game_id: str, num: int = 10) -> List[ChatMemory]:
        tmp = []
        if os.path.exists(file_path):
            with open(file_path, "r", encoding="utf-8") as file:
                ...
                for line in last_10_lines:
                    try:
                        ...
                        logger.exception(
                            f"error {e} in read_last_n_lines_with_json_loads for {line}",
                            exc_info=True,
                        )
                        pass
        return tmp

    def get_chat_memory(self, filter_id: str = "") -> List[ChatMemory]:
    ...
    @staticmethod
    def get_event_memory(game_info: Dict[str, Any], civ_ind: int):
        ...
        notifications_log = game_info["civilizations"][civ_ind].get("notificationsLog", [])

...
default_req = {
    ...
    "civ_name": "",
    "round": 0,
    "dev_setting": {"model_name": "gpt-3.5-turbo"},
    "speaker_persona": {
        "npc_id": "",
        "civ_name": "",
        "language_style": "",
...
def prompt_elem_mapping(info: List[Any], type_: str = "") -> str:
    if type_ == "strength":
...
def save2req(
    ...
    agent: CivAgent,
    text: str,
    speaker_civ_name: str,
    receiver_civ_name: str,
) -> Dict[str, Any]:
    # todo support two robot
    tmp_req: dict[str, Any] = copy.deepcopy(default_req)
    ...
    tmp_req["relation"]["diplomatic_status"] = agent.relations[f"{receiver_civ_name}#{speaker_civ_name}"][
        "diplomatic_status"
    ]

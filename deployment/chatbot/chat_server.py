...
def admin_logic(
    ...
    if gameid:
        ...
        #     platform=platform
        # )
        gameid2info: dict = mq.get("gameid2info_" + gameid, {})
        gameid2info["gameid"] = gameid
        gameid2info["platform"] = platform
        if gameinfo:
...
def reply(data: Dict[str, Any], admin_name: str, platform: str) -> None:
        ...
        try:
            ...
            if is_group:
                chatmessage = chat_manager.send_group_msg(
                    data["notify"], sender=data["fromBot"], platform=data["msgType"]
                )
            else:

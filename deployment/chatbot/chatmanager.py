...
from deployment.chatbot.discord.discord_chatbot import DiscordChatbot, discord_robot2id
...
class ChatManager:
    ...
    robot_names: List[str] = list(feishubot2id.keys()) + list(discord_robot2id.keys())

    def __init__(self, user_id: str):
        self.user_id: str = user_id
        self.platforms: List[str] = ["discord", "feishu"]
        self.chatbots: Dict[str, Dict[str, CivChatbot]] = {}
        self.last_session_id: str = ""
        self.last_uuid: str = ""
        self.game_id: str = ""
        self.game_info: Dict[str, str] = {}
    ...
    def get_teamid(self, platform: str) -> str:
        if platform == "discord":
    ...
    @staticmethod
    def get_teamid_static(game_id: str, user_id: str, platform: str) -> str:
        if platform == "discord":
    ...
    def update_from_redis(self, platform: str = "discord") -> None:
        ...
        if platform == "discord":
            ...
            for robot_name in discord_robot2id.keys():
                ...
                bot.team_id = team_id
                channel_id = mq.get(f"discord_channel_{self.user_id}_{robot_name}_private", "")
    ...
    def create_chatbots(self, platform: str) -> None:
        if platform == "discord":
    ...
    def send_msg(self, text: str, sender: str, receiver: str, platform: str) -> ChatMessage:
    ...
    def get_admin_chatbot(self, platform: str) -> CivChatbot:
        if platform == "discord":
    ...
    def get_chatbot(self, platform: str, robot_name: str) -> CivChatbot:
        if robot_name in self.chatbots[platform]:
            return self.chatbots[platform][robot_name]
        else:
            return self.get_admin_chatbot(platform)

    def send_group_msg(self, text: str, sender: str, platform: str, team_id: Optional[str] = None) -> ChatMessage:
        team_id = self.get_admin_chatbot(platform).team_id if team_id is None else team_id
        bot: CivChatbot = self.get_chatbot(platform, sender)
        self.update_chatbot(bot)
        return bot.send_group_msg(text, team_id)

    def update_chatbot(self, bot: CivChatbot) -> None:
        bot.last_session_id = self.last_session_id
        bot.last_uuid = self.last_uuid

    def convert_to_chatmessage(self, msg, platform: str) -> ChatMessage:
        if platform == "discord":
    ...
    @staticmethod
    def extract_message(msg, platform: str) -> Tuple[str, str, str]:
        if platform == "discord":
            chatmessage = DiscordChatbot.convert_to_chatmessage(msg)
        elif platform == "feishu":
            chatmessage = FeishuChatbot.convert_to_chatmessage(msg)
    ...
    def create_team(self, team_name: str, uids: List[str], platform: str) -> str:
        if platform == "discord":
    ...
    def get_chatmemory(self, chatmessage: ChatMessage) -> ChatMemory:
        ...
        from_civ = chatmessage.fromBot if chatmessage.fromBot in self.robot_names else player_civ[0]
        to_civ = chatmessage.toBot if chatmessage.toBot in self.robot_names or is_group else player_civ[0]
        tmp_d = {
            **asdict(chatmessage),
            "gameId": self.game_id,
            "userId": self.user_id,
    ...
    @staticmethod
    def get_reply_chatmemory(chatmemory: ChatMemory, text: str, is_group: Optional[bool] = None) -> ChatMemory:
        return ChatMemory(

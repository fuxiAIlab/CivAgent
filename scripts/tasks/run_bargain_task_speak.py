...
from civagent.utils.utils import default_req, save2req
...
file_handler = logging.FileHandler(f"../../Log/bargain_{current_time}.log")
...
def reply(agent, bargain_role, req, save_data, text, only_line=False):
    ...
    civ1_resource_dict, civ2_resource_dict, identify_result, _ = agent.extract_trades(
        "propose_trade", req, model="gpt-3.5-turbo-1106"
    )
    ...
    response = {}
    if agent.default_llm == "human":
        response["response"] = input("Please input your response:")
    else:
...
if __name__ == "__main__":
    arguments = sys.argv
    # arguments = 'none ../reproductions/Autosave-China-60 rome gpt3.5 china human'.split(' ')
    ...
    for i in range(10):
        ...
        buyer_line, buyer_identify_result, buyer_border_info = reply(
            agent_buyer, "buyer", req_buyer, save_data, text, only_line=True
        )
        ...
        seller_line, seller_identify_result, seller_border_info = reply(
            agent_seller, "seller", req_seller, save_data, text, only_line=True
        )
        ...
        for j in range(5):
            ...
            if response.get("bargain_result", "").lower() == "yes" or check_result:
                ...
                result[seller_civ] = (
                    (abs(Price - Market_price_bottom) / abs(Market_price_top - Market_price_bottom))
                    + result[seller_civ] * success_num
                ) / (success_num + 1)
                result[buyer_civ] = (
                    (abs(Market_price_top - Price) / abs(Market_price_top - Market_price_bottom))
                    + result[buyer_civ] * success_num
                ) / (success_num + 1)
                success_num += 1
                break

...
def run_benchmark(reflection_mode, save_path, turns, key="declare_war", config_data={}):
    ...
    for i in range(turns):
        ...
        for robot_name in robot_names:
                ...
                if proposals is not None and len(proposals) > 0:
                    for proposal in proposals:
                        ...
                        score = (
                            utils.get_stats(
                                simulator_save_data,
                                utils.get_civ_index(simulator_save_data, robot_name),
                            )["civ_strength"]
                    ...
                    prompt, llm_config = prompt_make("agent_react", context_dict={**req, "skill_info": bot_skills})
                    req["llm_config"] = llm_config
                    req["llm_config"]["skill_info"] = bot_skills
                    ...
                    for proposal in proposals:
                        for proposal_before in proposals_before:
                            if proposal.get("skill_name", "") == proposal_before.get("skill_name", ""):
            ...
            if proposals is not None and len(proposals) > 0:
                for proposal in proposals:
                    if proposal.get("skill_name", "") == "cheat":
                        to_civ_agent = agents[proposal["to_civ"]]
                        req_to_civ = save2req(
                            save_data,
                            agents[proposal["to_civ"]],
                            text="",
                            speaker_civ_name=robot_name,
                            receiver_civ_name=proposal["to_civ"],
                        )
                        req_to_civ["speak_content"] = proposal["param"]["fake_news"]
                        recognition_prompt, llm_config = prompt_make(
                            "agent_recognition",
                            context_dict={**req_to_civ, **{"proposal": proposal}},
                        )
                        req_to_civ["llm_config"] = llm_config
                        decision, _, _ = workflow_utils.run_workflows(
                            req={"prompt": recognition_prompt, **req_to_civ},
                            is_reply=True,
                            model=config_data[proposal["to_civ"]]["model"],
                        )
                        recognition_decision = decision["Decision"]
                        fakenew = (
                            f"""At {turn} -{robot_name} says to you: {proposal['param']['fake_news']}."""
            ...
            if proposals is not None and len(proposals) > 0:
                for proposal in proposals:
                    ...
                    to_civ, _ = proposal["to_civ"], utils.get_civ_index(save_data, proposal["to_civ"])
                    req_to_civ["short_term"] = agents[proposal["to_civ"]].short_term
                    event_type = proposal["skill_name"]
                    logger.debug(
                        f"""On the {turn} turn, {robot_name} 's civilization power was  {score}, technological force is {tech_score}, """
                    ...
                    while trynum < 3:
                        try:
                            decision, _, _ = workflow_utils.run_workflows(
                                ...
                                force_json=False,
                                model=config_data[to_civ]["model"],
                                is_reply=True,
                    ...
                    proposal_yes_str = PromptHub.diplomatic_memory[event_type].format(
                        **proposal["param"], decision_str="agree"
                    )
                    proposal_no_str = PromptHub.diplomatic_memory[event_type].format(
                        **proposal["param"], decision_str="reject"
                    )
                    proposal_yes_str_oppo = PromptHub.diplomatic_memory_oppo[event_type].format(
                        **proposal["param"], decision_str="agree"
                    )
                    proposal_no_str_oppo = PromptHub.diplomatic_memory_oppo[event_type].format(
                        **proposal["param"], decision_str="reject"
                    )
                    if decision == "yes" or "yes" in decision or event_type == "declare_war":
                        ...
                        if key == "change_closeness":
                            agent.relations[f"{agent.civ_name}#{to_civ}"]["closeness"] = proposal["param"][
                                "next_relation"
                            ]
                            agent.relations[f"{agent.civ_name}#{to_civ}"]["expected_closeness"] = proposal["param"][
                                "next_relation"
                            ]
                            agents[to_civ].relations[f"{to_civ}#{agent.civ_name}"]["history_event"][
                                -1
                            ] = proposal_yes_str
                        ...
                        agents[to_civ].relations[f"{to_civ}#{agent.civ_name}"]["history_event"].append(
                            proposal_no_str_oppo
                        )

        simulator.init_jvm()
        save_data = simulator.run(save_data, turns=5, diplomacy_flag=False, worker_auto=True)
        ...
        with open(log_file_path, "r") as file:
            for line in file:
                if keyword in line and civ_keyword in line:
                    ...
                    prompt_reflection, llm_config = prompt_make("agent_reflection", context_dict={**req})
